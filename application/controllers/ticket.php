...
class Ticket extends CI_Controller
{
	...
	function save()
	{
		...
		// join name of image using comma

		$getkodeticket = $this->model_app->getkodeticket();

		$ticket = $getkodeticket;
		$target_dir_berita_acara = "assets/berita_acara/";
		$berita_acara_path = $_FILES['berita_acara']['name'];
		$ext1 = pathinfo($berita_acara_path, PATHINFO_EXTENSION);
		$target_file = $target_dir_berita_acara . basename($ticket . '-berita_acara' . '.' . $ext1);
		move_uploaded_file($_FILES['berita_acara']["tmp_name"], $target_file);
		...
		for ($i = 0; $i < $count; $i++) {
			if ($count != 0) {
				$path = $_FILES['lampiran']['name'][$i];
				$ext = pathinfo($path, PATHINFO_EXTENSION);
				$target_file = $target_dir . basename($ticket . '-' . $i . '.' . $ext);
				move_uploaded_file($_FILES['lampiran']["tmp_name"][$i], $target_file);
				$lampiran .= $ticket . '-' . $i . '.' . $ext . ',';
			}
		}
		...
		$data['berita_acara'] = $ticket . '-berita_acara' . '.' . $ext1;
