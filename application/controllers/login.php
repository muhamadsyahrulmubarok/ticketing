...
class Login extends CI_Controller
{
	...
	function login_akses()
	{
		...
		$akses = $this->db->query("select A.username, B.nama, A.level, B.id_jabatan, C.id_dept, A.id_karyawan FROM user A 
		LEFT JOIN karyawan B ON B.nik = A.id_karyawan
	 ...
	 WHERE A.username = '$username' AND A.password = '$password'");
		...
		if ($akses->num_rows() == 1) {
			...
			foreach ($akses->result_array() as $data) {
				$session['id_user'] = $data['id_karyawan'];
